<?php
	...
	if($error == 0){
		...
		//Penerimaan
		$queryPenerimaan = sprintf("SELECT * FROM `penerimaan` WHERE tanggal <= '%04d-%02d-01'", $th, $bln);
		$sql1 = $mysqli->query($queryPenerimaan);
		...
		$likeFilter = sprintf("'%04d-%02d-__'", $th, $bln);
		
		$queryAkunPenerimaan =
			"SELECT p.id_akun,a.namaakun,SUM(p.jumlah) as jumlah FROM penerimaan p, akun a ".
			"WHERE p.id_akun=a.kode AND tanggal LIKE " .$likeFilter . " AND id_akun LIKE '1.%' ".
			"GROUP BY p.id_akun";
		$sqls = $mysqli->query( $queryAkunPenerimaan );
		...
		$queryAkunPengeluaran = 
			"SELECT p.id_akun,a.namaakun,SUM(p.jumlah) as jumlah FROM penyaluran p, akun a WHERE ".
			"p.id_akun=a.kode AND tanggal LIKE " .$likeFilter . " AND (id_akun LIKE '2.%') ".
			"GROUP BY p.id_akun";
		$sqls = $mysqli->query($queryAkunPengeluaran);
		$i=0;
		$total_keluar = 0;
		while($q = $sqls->fetch_array(MYSQLI_ASSOC)) {
			$i++;
		...
		$queryAkunPengeluaran =
			"SELECT p.id_akun,a.namaakun,SUM(p.jumlah) as jumlah FROM penyaluran p, pengeluaran a ".
			"WHERE p.id_akun=a.kode AND tanggal LIKE " .$likeFilter . " AND (id_akun LIKE '4.%') ".
			"GROUP BY p.id_akun";
		$sqls = $mysqli->query( $queryAkunPengeluaran );
		$total_keluar1 = 0;
