				...
				<div class="form-group">
					...
					<div class="controls">
						<select data-placeholder="Pilih Akun Penyaluran" name='akun_<?php echo $p1['idakun']?>[]' style="width:350px;" multiple class="chzn-select" tabindex="8">
							<option></option>
							<?php
								$sql = mysql_query("SELECT * FROM akun WHERE jenis='2'");
								while($s = mysql_fetch_array($sql)){
									$sqla1 = mysql_query("SELECT * FROM persamaan_akun WHERE id_penerimaan='$p1[kode]' AND id_penyaluran='$s[kode]'");
									if(mysql_num_rows($sqla1) > 0){
										echo "<option value='$s[kode]' selected>$s[kode] $s[namaakun]</option>";
									}else{
										echo "<option value='$s[kode]'>$s[kode] $s[namaakun]</option>";
									}
								}
								$sql = mysql_query("SELECT * FROM pengeluaran WHERE jenis='4'");
								while($s = mysql_fetch_array($sql)){
									$sqla1 = mysql_query("SELECT * FROM persamaan_akun WHERE id_penerimaan='$p1[kode]' AND id_penyaluran='$s[kode]'");
									if(mysql_num_rows($sqla1) > 0){
										echo "<option value='$s[kode]' selected>$s[kode] $s[namaakun]</option>";
				...
				<div class="form-group">
					...
					<div class="controls">
						<select data-placeholder="Pilih Akun Penyaluran" name='akun_<?php echo $p1['idakun']?>[]' style="width:350px;" multiple class="chzn-select" tabindex="8">
							...
							<?php
								$sql = mysql_query("SELECT * FROM akun WHERE jenis='3' AND kode LIKE '3.2%'");
								while($s = mysql_fetch_array($sql)){
									$sqla1 = mysql_query("SELECT * FROM persamaan_akun WHERE id_penerimaan='$p1[kode]' AND id_penyaluran='$s[kode]'");
									if(mysql_num_rows($sqla1) > 0){
										echo "<option value='$s[kode]' selected>$s[kode] $s[namaakun]</option>";
