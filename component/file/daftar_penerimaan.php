				...
				<table>
					<tr valign='top'>
						...
						<td><div style='font-size:15px;font-weight:bolder;'><?php
							$ttl = mysqli_query($mysqli, "SELECT SUM(jumlah) as jml FROM penerimaan WHERE tanggal LIKE '__-__-$_GET[th]'");
					...
					<tr valign='top'>
						...
						<td>&nbsp;&nbsp;:</td>
						<td><div style='font-size:15px;font-weight:bolder;'><?php
							$ttl = mysqli_query($mysqli, "SELECT count(id_penerimaan) as jml FROM penerimaan WHERE tanggal LIKE '__-__-$_GET[th]'");
							$ttla = mysqli_fetch_array($ttl);
							echo $ttla['jml'];
						?></div></td>
					</tr>
						...
						while($p = mysqli_fetch_array($q1)){
							...
							echo "<tr>";
								...
								foreach($month as $m => $val){
									$sql = mysqli_query($mysqli, "SELECT SUM(jumlah) as jumlah FROM penerimaan WHERE id_akun='$p[kode]' AND MID(tanggal,4,2) = '$m' AND MID(tanggal,7,4) = '$_GET[th]'");
