...
// Output: JSON
//---------------------------
// Proses mengedit record stage transaksi pengeluaran
// Note: field input sesuai dengan AM_SIZ_STG_PENGELUARAN
	...
	if (isset($_POST['id'])) {
		...
		$idDonatur	= trim($_POST['penerima']);
		$idAmilin	= trim($_POST['pj']);
		...
		if ($processError == null) {
			$queryCheck = sprintf("SELECT * FROM stage_pengeluaran WHERE id_stage=%d", $stageId);
		...
		if ($processError == null) {
			...
			if ($errMsg) {
				...
				} else if (!empty($noNota) && !preg_match("/^[0-9-\s]+$/", $noNota)){
				...
				} else if (!empty($idAmilin) && !ctype_digit($idAmilin)) {
		...
		//== Cek amilin
		if (($processError == null) && (!empty($idAmilin))) {
			$dataAmilin = cek_user($idAmilin);
			if ($dataAmilin != null) {
				if ($dataAmilin['level'] != 99) {
					$processError = "ID petugas tidak valid.";
				}
			} else {
				$processError = "Data petugas tidak ditemukan.";
			}
		}
		...
		if (empty($processError)) {
			...
			// Update record
			$queryUpdate = "UPDATE stage_pengeluaran SET ".$querySet." WHERE id_stage=".$stageId;
			...
			if ($resultUpdate == null) {
				...
				$resultReFetch = mysqli_query($mysqli,
						"SELECT s.*, a.namaakun ".
						"FROM (SELECT * FROM stage_pengeluaran WHERE id_stage=".$stageId.") AS s ".
				...
				$htmlRecord = getHTMLRowTrxPengeluaran($dataTrx);
			}
