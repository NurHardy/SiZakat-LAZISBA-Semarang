<?php
/*
 * mapping_penerimaan.php
 * ==> Tampilan untuk mengatur memapping akun, donatur, amilin, bank, dsb.
 *
 * AM_SIZ_MAP_PENERIMAAN | Tampilan mapping penerimaan
 * ------------------------------------------------------------------------
 */
...
if ($colName == 'akun') {
	...
	$ajaxSuggestAct = "get.akun.penerimaan";
	$templateFuncName = "formatItemAkun";
	
	$queryGetMapSource =
		"SELECT ket_akun, NULL AS meta_info, COUNT(ket_akun) AS jumlah FROM stage_penerimaan " .
	...
	$currentColumn = "donatur";
	$ajaxSuggestUrl = "main.php?s=ajax&m=user";
	$ajaxSuggestAct = "get.user.donatur";
	$templateFuncName = "formatItemDonatur";
	
	$queryGetMapSource =
		"SELECT nama_donatur AS ket_akun, alamat_donatur AS meta_info, COUNT(nama_donatur) AS jumlah FROM stage_penerimaan " .
...
} else if ($colName == 'amilin') {
	$currentColumn = "amilin";
	$ajaxSuggestUrl = "main.php?s=ajax&m=user";
	$ajaxSuggestAct = "get.user.amilin";
	$templateFuncName = "formatItemDonatur";

	$queryGetMapSource =
	"SELECT nama_amilin AS ket_akun, NULL AS meta_info, COUNT(nama_amilin) AS jumlah FROM stage_penerimaan " .
	"WHERE id_teller='0' GROUP BY nama_amilin ";
	...
	$queryGetMapSource =
		"SELECT nama_bank AS ket_akun, NULL AS meta_info, COUNT(nama_bank) AS jumlah FROM stage_penerimaan " .
...
function submit_map_penerimaan(mapId) {
	var formFields = $("form#mapper_id_"+mapId).serialize();

	_ajax_send("act=map.stagepenerimaan&col="+ACTIVE_COLUMN+"&"+formFields, function(data){
...
while ( $rowMapSource = mysqli_fetch_assoc ( $resultGetMapSource ) ) {
	...
	echo "<form action=\"#save\" method=\"post\" id=\"mapper_id_".$mapId."\"
				onsubmit=\"return submit_map_penerimaan(".$mapId.");\">\n";
