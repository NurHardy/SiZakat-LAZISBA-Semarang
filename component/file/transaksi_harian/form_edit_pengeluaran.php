<?php
/*
 * form_edit_pengeluaran.php
 * ==> Tampilan form edit dan proses menyimpan penyaluran/pengeluaran
 *
 * AM_SIZ_FRMEDITPENGELUARAN | Tampilan form edit pengeluaran
 * ------------------------------------------------------------------------
 */

	$SIZPageTitle = "Edit Transaksi Pengeluaran";
	$breadCrumbPath[] = array("Edit Pengeluaran","main.php?s=form_dana_zakat_s",true);
	...
	if (isset($_POST['submit'])) {
		require COMPONENT_PATH."/file/transaksi_harian/functions/simpan_pengeluaran.php";
		...
		if ($isEditing) {
			$idPengeluaran = intval ( $_GET ['id'] );
			$queryGet = sprintf ( "SELECT * FROM penyaluran WHERE id_penyaluran=%d", $idPengeluaran );
			...
			if ($transactionData == null) {
				$errorDescription = "Data transaksi pengeluaran tidak ditemukan dalam database.";
				include COMPONENT_PATH . "/file/pages/error.php";
				return;
			}
			...
			$trxData ['id_trx']	= $transactionData ['id_penyaluran'];
		} else {
...
<?php if ($showForm) {//=========== TAMPILKAN FORM? ============= ?>
	...
	<?php if ($isEditing) { //---------------------------- ?>
					<div class="form-row control-group row-fluid form-group">
						<label class="control-label span3" for="normal-field">ID Transaksi Keluar</label>
