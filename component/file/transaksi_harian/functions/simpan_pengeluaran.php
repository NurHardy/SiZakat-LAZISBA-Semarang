...
// Note: field input sesuai dengan AM_SIZ_FRMPENGELUARAN
	...
	if (isset($_POST['submit'])) {
		...
		//== Simpan POST untuk kembali ditampilkan dalam form
		if (!isset($trxData)) $trxData = array();
		$trxData ['tanggal'] = $tglTrx;
		$trxData ['jumlah'] = $nominalTrx;
		$trxData ['nota'] = $noNota;
		$trxData ['id_donatur'] = $idDonatur;
		$trxData ['id_petugas'] = $idAmilin;
		$trxData ['kode_akun'] = $kodeAkunTrx;
		...
		if (empty($processError)) {
			...
			if (!$isEditing) {
				$updateFields['tgl_entry'] = $currentDate;
				$updateFields['id_creator'] = $_SESSION['iduser'];
			} else {
				$updateFields['tgl_update'] = $currentDate;
				$updateFields['id_editor'] = $_SESSION['iduser'];
			}
