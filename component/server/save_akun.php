<?php
	...
	if(ISSET($_POST['submit'])){
		...
		$data = mysql_query("SELECT * FROM akun WHERE namaakun='$nama' AND idParent = '$induk'");
		if(mysql_num_rows($data) == 0){
			/*Mengambil data induk*/
			$sql1 = mysql_query("SELECT * FROM akun WHERE idakun='$induk'");
			...
			$sql1 = mysql_query("SELECT * FROM akun WHERE kode LIKE '$fa[kode]%' AND idParent='$fa[idakun]' ORDER BY kode DESC LIMIT 0,1");
