<?php
	...
	if(ISSET($_POST['save'])){
		...
		$data = mysqli_query($mysqli, "SELECT * FROM pengeluaran WHERE namaakun='$nama' AND idParent = '$induk'");
		if(mysqli_num_rows($data) == 0){
			/*Mengambil data induk*/
			$sql1 = mysqli_query($mysqli, "SELECT * FROM pengeluaran WHERE idakun='$induk'");
			...
			$sql1 = mysqli_query($mysqli, "SELECT * FROM pengeluaran WHERE kode LIKE '$fa[kode]%' AND idParent='$fa[idakun]' ORDER BY kode DESC LIMIT 0,1");
